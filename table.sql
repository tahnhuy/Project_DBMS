...
    Unit           NVARCHAR(50)   NOT NULL,
    -- COLUMN ĐƯỢC THÊM VÀO CHO TÍNH NĂNG SOFT DELETE (XÓA MỀM)
...
    Role        NVARCHAR(20)   NOT NULL CHECK (Role IN ('manager', 'saler')), -- ĐÃ SỬA: Bỏ 'customer'
...
        -- ĐÃ SỬA: Chỉ chấp nhận Role manager/saler nếu EmployeeID IS NOT NULL
        Role IN ('manager', 'saler') AND EmployeeID IS NOT NULL AND CustomerID IS NULL
        -- (Đã bỏ điều kiện OR cho 'customer' role)
    )
);
GO
